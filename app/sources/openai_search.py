"""OpenAI-powered job search using web search capabilities."""
...
class OpenAISearchProvider:
    """Search provider using OpenAI with web browsing."""
    ...
    def search(
        ...
        """Search for underclass internships using OpenAI.
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o",  # or gpt-4-turbo with browsing
                max_tokens=4096,
                messages=[
                    {
                        "role": "system",
                        "content": "You are a job search assistant. Search the web and return structured JSON results."
                    },
    ...
    def _parse_results(self, content: str) -> list[Posting]:
        """Parse OpenAI response into Posting objects."""
        postings = []
        ...
        logger.info(f"OpenAI search found {len(postings)} postings")
        return postings
